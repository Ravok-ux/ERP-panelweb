...
// clientes.js â€” Directorio de clientes con KPIs, bÃºsqueda y ficha detalle
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
// â”€â”€ Focus trap utility â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Columnas Excel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const _COLS = [
  { key: "nombre",       header: "Nombre",           width: 30, required: true },
  { key: "telefono",     header: "TelÃ©fono",         width: 16 },
  { key: "direccion",    header: "DirecciÃ³n",        width: 36 },
  ...
  { key: "ultimaVisita", header: "Ãšltima visita",    width: 18, fmt: "fecha" },
...
let _clientes   = [];
let _filtrados  = [];
let _abcMap     = {};  // clienteDocId â†’ 'A'|'B'|'C'
let _abcCargado = false;

// â”€â”€ Filtros activos â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let _fBusqueda  = "";
...
// â”€â”€ MÃ³dulo exportado â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function _html() {
  ...
  <div style="padding:0 0 24px">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:10px;margin-bottom:14px">
      ...
      ${_kpi("cli-k-abc-a",  "CATEGORÃA A",    "#B45309")}
    </div>
    ...
    <div style="background:var(--surface);border-radius:10px;border:1px solid var(--border);
      ...
      <button onclick="ClientesUI.exportar()"
        ...
        style="padding:7px 11px;background:transparent;color:var(--text-sec);border:1px solid var(--border);border-radius:6px;cursor:pointer;font-size:12px;white-space:nowrap">â¬‡ï¸ Excel</button>

      <!-- Importar -->
      <button onclick="ClientesUI.importarExcel()"
        title="Importar clientes desde Excel"
        style="padding:7px 11px;background:transparent;color:var(--text-sec);border:1px solid var(--border);border-radius:6px;cursor:pointer;font-size:12px;white-space:nowrap">â¬†ï¸ Importar</button>
    ...
    <div style="background:var(--surface);border-radius:10px;border:1px solid var(--border);
      ...
      <div id="cli-scroll-outer" style="overflow-x:auto;overflow-y:auto;max-height:calc(100vh - 330px)">
        <table style="width:100%;border-collapse:collapse;font-size:12px" id="cli-table">
          ...
          <tbody id="cli-tbody">
            <tr><td colspan="9" style="padding:32px;text-align:center;color:#9CA3AF">Cargando clientesâ€¦</td></tr>
          </tbody>
        </table>
      </div>
    </div>
  ...
  <!-- â”€â”€ Modal configuraciÃ³n de columnas â”€â”€ -->
  <div id="cli-modal-cols" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);
    ...
    <div style="background:var(--surface);border-radius:14px;width:360px;max-width:100%;
      ...
      <div style="font-size:11px;color:#9CA3AF;margin-bottom:16px">Arrastra para reordenar Â· desmarca para ocultar</div>
  ...
  <!-- â”€â”€ Modal ubicaciÃ³n con Google Maps â”€â”€ -->
  <div id="cli-ubic-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);
    ...
    <div style="background:var(--surface);border-radius:14px;width:680px;max-width:100%;
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px">
        <span id="cli-ubic-titulo" style="font-size:15px;font-weight:800;color:var(--text-primary)">Nueva ubicaciÃ³n</span>
        <button onclick="ClientesUI.cerrarUbicacion()"
          aria-label="Cerrar"
          style="border:none;background:transparent;font-size:20px;cursor:pointer;color:#9CA3AF">âœ•</button>
      </div>
      ...
      <p style="font-size:11px;color:#6B7280;margin:0 0 14px">
        Haz clic en el mapa o arrastra el marcador para fijar la posiciÃ³n exacta.
      </p>
      ...
      <div id="cli-ubic-compartido-wrap" style="border:1px solid var(--border);border-radius:8px;
        ...
        <label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-bottom:0">
          ...
          <span style="font-size:11px;font-weight:600;color:var(--text-primary)">
            Este cliente opera en zonas de mÃ¡s de un ingeniero
          </span>
        </label>
  ...
  <!-- â”€â”€ Modal fotos del cliente (APK) â”€â”€ -->
  <div id="cli-fotos-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);
    ...
    <div style="background:var(--surface);border-radius:14px;width:700px;max-width:100%;
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
        <div>
          <div id="cli-fotos-title" style="font-size:15px;font-weight:800;color:var(--text-primary)">ðŸ“· Fotos del cliente</div>
  ...
  <!-- â”€â”€ Modal Nuevo/Editar Cliente â”€â”€ -->
  <div id="cli-form-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);
  ...
  <div id="cli-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);
...
function _kpi(id, label, color = "var(--text-primary)") {
  return `<div style="background:var(--surface);border-radius:10px;border:1px solid var(--border);
    ...
    <div style="font-size:20px;font-weight:800;color:${color};font-variant-numeric:tabular-nums" id="${id}">â€“</div>
    <div style="font-size:11px;font-weight:600;color:var(--text-sec);margin-top:2px">${label}</div>
  </div>`;
}

// â”€â”€ Bind UI â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function _bindUI() {
  window.ClientesUI = {
    ...
    sortBy(col) {
      ...
      ["nombre","segmento","ingeniero","zona","saldo","ultimaVisita"].forEach(k => {
        ...
        if (el) el.textContent = _sortCol === k ? (_sortDir === 1 ? "â–²" : "â–¼") : "";
      });
      _applyFilters();
    },
    ...
    async _selGeoResult(placeId, desc) {
      ...
      try {
        ...
        svc.getDetails({ placeId, fields: ["geometry","formatted_address","address_components"] }, (place, st) => {
          ...
          // Rellenar campos de direcciÃ³n si estÃ¡n vacÃ­os
          ...
          window.toast?.("UbicaciÃ³n encontrada y coordenadas aplicadas.", "success");
        });
      } catch(e) { window.toast?.("Error buscando ubicaciÃ³n.", "error"); }
    },
    ...
    geolocalizarCliente() {
      ...
      navigator.geolocation.getCurrentPosition(pos => {
        ...
        if (elLat) elLat.value = lat.toFixed(7);
        if (elLng) elLng.value = lng.toFixed(7);
        window.toast?.("Coordenadas de tu ubicaciÃ³n actual aplicadas.", "success");
      }, () => window.toast?.("No se pudo obtener la ubicaciÃ³n GPS.", "error"));
    },
    ...
    abrirConfigCols() {
      ...
      list.innerHTML = ordered.map(key => {
        ...
        return `<li data-col="${key}" draggable="${!meta.locked}"
          ...
          <span style="font-size:14px;color:#9CA3AF">${meta.locked ? "âŠ" : "â ¿"}</span>
          <span style="flex:1;font-size:12px;font-weight:600;color:var(--text-primary)">${meta.label||"â€”"}</span>
          <input type="checkbox" ${visible?"checked":""} ${meta.locked?"disabled":""} style="cursor:pointer;width:16px;height:16px">
        </li>`;
      }).join("");
      // Drag & drop â€” pointer events (funciona en tÃ¡ctil y mouse)
      let _dragSrc = null;
    ...
    async eliminarUbicacion(clienteDocId, ubicId) {
      if (!await window.modal?.({ title:"Eliminar ubicaciÃ³n", message:"Â¿Confirmas eliminar esta ubicaciÃ³n? Esta acciÃ³n no puede deshacerse." })) return;
      try {
        const { doc: d2, deleteDoc } =
          await import("https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js");
        await deleteDoc(d2(db, "clientes", clienteDocId, "ubicaciones", ubicId));
        window.toast?.("UbicaciÃ³n eliminada.", "success");
  ...
  // SincronizaciÃ³n de doble scroll (top â†” bottom)
...
// â”€â”€ Firestore listener â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function _escuchar() {
...
// â”€â”€ Poblar selects de segmento e ingeniero â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Aplicar filtros y renderizar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function _renderKPIs() {
  ...
  set("cli-k-abc-a")(abcA > 0 ? abcA.toLocaleString("es-MX") : "â€“");
...
// â”€â”€ Tabla â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function _renderTabla() {
  ...
  tbody.innerHTML = _filtrados.map((c, i) => {
    ...
    const saldoTxt   = saldo > 0 ? fmt.format(saldo) : "â€”";
    ...
    return `<tr style="border-bottom:1px solid var(--border);
      ...
      <td style="padding:10px 14px">
        <div style="font-weight:700;font-size:12px;color:var(--text-primary)">
          ${esc(c.nombre || "â€”")}${abcBadge}
          ${clienteEnBlacklist(c.id, c.nombre) ? `<span title="Cliente en lista negra" style="margin-left:4px;background:#FEE2E2;color:#DC2626;font-size:10px;font-weight:800;padding:1px 6px;border-radius:4px;border:1px solid #FECACA;vertical-align:middle">ðŸš« BLACKLIST</span>` : ""}
        </div>
      ...
      <td style="padding:10px 14px;font-size:12px;color:var(--text-sec)">${esc(c.ingeniero || "â€”")}</td>
      <td style="padding:10px 14px;font-size:11px;color:#6B7280">${esc(c.zona || "â€”")}</td>
      <td style="padding:10px 14px;text-align:right;font-size:12px;font-weight:700;color:${saldoColor}">
        ...
        <div style="font-size:9px;color:${creditoCol};margin-top:1px;font-weight:600">${creditoPct}% crÃ©dito</div>` : ""}
      </td>
...
// â”€â”€ Colores de segmento â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Detalle de cliente â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function _abrirDetalle(id) {
  ...
  body.innerHTML = `<div style="padding:20px;text-align:center;color:#9CA3AF">Cargandoâ€¦</div>`;
  modal.style.display = "flex";
  document.body.style.overflow = "hidden";
  _openTrap("cli-modal", () => window.ClientesUI.cerrarDetalle());

  // Buscar en cachÃ© local primero
  ...
  body.innerHTML = `
    ...
    <div style="background:${_blColor}10;border:2px solid ${_blColor};border-radius:10px;padding:14px 18px;margin-bottom:18px;display:flex;align-items:flex-start;gap:12px">
      <span style="font-size:22px;flex-shrink:0">ðŸš«</span>
      <div style="flex:1">
        <div style="font-size:13px;font-weight:800;color:${_blColor};margin-bottom:3px">
          CLIENTE EN LISTA NEGRA â€” RIESGO ${esc(_blEntry.nivelRiesgo||"ALTO")} Â· ${esc(_blEntry.categoria||"")}
        </div>
        <div style="font-size:12px;color:var(--text-pri);line-height:1.5">
          ${esc((_blEntry.descripcion||"").slice(0, 200))}${(_blEntry.descripcion||"").length > 200 ? "â€¦" : ""}
        </div>
    ...
    <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:20px">
      <div style="display:flex;align-items:center;gap:14px">
        ...
        <div>
          <div style="font-size:16px;font-weight:800;color:var(--text-primary)">${esc(c.nombre || "â€”")}</div>
          <div style="font-size:12px;color:#6B7280;margin-top:2px">
            ${c.clienteId ? `<span style="font-family:monospace;font-weight:700;color:var(--text-sec)">${esc(c.clienteId)}</span> Â· ` : ""}
            ${c.segmento ? `<span style="font-weight:600;color:${segCol}">${esc(c.segmento)}</span> Â· ` : ""}
          ...
          <div style="display:flex;align-items:center;gap:6px;margin-top:6px;flex-wrap:wrap">
            <span style="font-size:10px;font-weight:700;padding:2px 8px;border-radius:9px;
              background:rgba(37,99,235,0.15);color:var(--blue,#2563EB);border:1px solid rgba(37,99,235,0.3)">ðŸ”— COMPARTIDO</span>
      ...
      <button onclick="ClientesUI.cerrarDetalle()"
        ...
        style="border:none;background:transparent;font-size:20px;cursor:pointer;
          color:#9CA3AF;padding:4px 8px;border-radius:6px">âœ•</button>
    </div>

    <!-- Grid de datos -->
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;margin-bottom:18px">
      ${_campo("ðŸ“ž TelÃ©fono", c.telefono)}
      ${_campo("ðŸ‘· Ingeniero", c.ingeniero)}
      ${_campo("ðŸ“ Zona", c.zona)}
      ${_campo("ðŸ™ Ciudad", [c.colonia, c.ciudad].filter(Boolean).join(", ") || null)}
      ${_campo("ðŸ  DirecciÃ³n", [c.calle, c.numExt ? `#${c.numExt}` : null].filter(Boolean).join(" ") || c.direccion || null)}
      ${_campo("ðŸ“… Ãšltima visita", fmtDt(c.fechaUltimaVisita || c.ultimaVisita))}
      ${c.tipo        ? _campo("ðŸ— Tipo de instalaciÃ³n", c.tipo)        : ""}
      ${c.tipoCultivo ? _campo("ðŸŒ± Tipo de cultivo",     c.tipoCultivo) : ""}
    </div>

    <!-- Saldo / LÃ­nea de crÃ©dito (Medio/56) -->
    ${(() => {
      ...
      <div style="border:1px solid var(--border);border-radius:10px;padding:14px 16px;
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:${lim>0?8:0}px">
          <div>
            <span style="font-size:12px;font-weight:600;color:${saldo>0?"#DC2626":"#16A34A"}">
              ${saldo > 0 ? "âš ï¸ Saldo pendiente" : "âœ… Sin saldo"}
            </span>
        ...
        <div>
          <div style="display:flex;justify-content:space-between;font-size:10px;color:#64748B;margin-bottom:4px">
            <span>LÃ­nea de crÃ©dito: <strong style="color:var(--text-primary)">${fmt.format(lim)}</strong></span>
          ...
          ${pct >= 90 ? `<div style="font-size:10px;color:#DC2626;font-weight:700;margin-top:4px">
            âš ï¸ LÃ­mite de crÃ©dito casi agotado â€” revisar antes de nuevos pedidos</div>` : ""}
        </div>` : `
        <div style="font-size:10px;color:#94A3B8;margin-top:2px">Sin lÃ­nea de crÃ©dito configurada</div>`}
      </div>`;
    })()}
    ...
    <div style="margin-bottom:16px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
        <span style="font-size:12px;font-weight:700;color:var(--text-primary)">ðŸ§¾ Pedidos recientes</span>
      </div>
    ...
    <div style="margin-bottom:16px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
        <span style="font-size:12px;font-weight:700;color:var(--text-primary)">ðŸ“ Ubicaciones</span>
      ...
      <div id="cli-ubicaciones-list" style="display:flex;flex-direction:column;gap:8px">
        <div style="padding:16px;text-align:center;color:#9CA3AF;font-size:12px">Cargandoâ€¦</div>
      </div>
    </div>
    ...
    <div style="margin-bottom:16px">
      ...
      ${c.notaWebActualizado
        ? `<div style="font-size:10px;color:#9CA3AF;margin-top:3px">
            Ãšltima ediciÃ³n: ${fmtDt(c.notaWebActualizado)} por ${esc(c.notaWebPor || "â€”")}
          </div>`
        : ""}
    </div>
    ...
    <div style="margin-bottom:16px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
        <span style="font-size:12px;font-weight:700;color:var(--text-primary)">ðŸ“… Seguimiento de visitas</span>
  ...
  // Cargar secciones asÃ­ncronas tras renderizar el modal
...
function _campo(label, valor) {
  return `<div style="background:var(--surface-2);border-radius:8px;padding:10px 12px;
    ...
    <div style="font-size:12px;font-weight:600;color:var(--text-primary)">${esc(valor) || `<span style="color:#D1D5DB">â€”</span>`}</div>
  </div>`;
}

// â”€â”€ DefiniciÃ³n de columnas (mismo patrÃ³n que productos-control) â”€â”€
const CLI_ALL_COLS = [
  ...
  { key:"visita",    label:"ÃšLTIMA VISITA"                },
...
// â”€â”€ Importar Excel (hoja 11 Coordenadas del APK) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const _IMPORT_MAP = {
  "Nombre":        "nombre",
...
async function _importarExcel(file) {
  ...
  window.toast?.("Leyendo archivoâ€¦", "info");
  ...
  const rows = XLSX.utils.sheet_to_json(ws, { header: 1, defval: "" });
  if (rows.length < 2) { window.toast?.("Hoja vacÃ­a.", "warn"); return; }
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    headers.forEach((h, idx) => {
      ...
      if (!campo) return;
      let val = row[idx];
      if (val === null || val === undefined || val === "â€”" || val === "") return;
  ...
  if (!records.length) { window.toast?.("Sin registros vÃ¡lidos.", "warn"); return; }
    ...
    overlay.innerHTML = `
      <div style="background:var(--surface);border-radius:14px;width:420px;max-width:100%;
        ...
        <div style="font-size:12px;color:var(--text-sec);margin-bottom:16px">
          Se importarÃ¡n <strong>${records.length}</strong> clientes de la hoja Â«${esc(wsName)}Â».<br>
          Selecciona el ingeniero al que se asignarÃ¡n (solo roles Ingeniero).
        </div>
        <div style="margin-bottom:16px">
  ...
  if (ingenieroDefault === null) return; // usuario cancelÃ³
  ...
  window.toast?.(`Importando ${records.length} registrosâ€¦`, "info");
  ...
  window.toast?.(
    `âœ… ImportaciÃ³n: ${creados} nuevos, ${actualizados} actualizados${errores ? ", " + errores + " errores" : ""}.`,
    errores ? "warn" : "success"
  );
}

// â”€â”€ Nuevo / Editar Cliente â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let _ingenierosList = [];
...
async function _abrirFormCliente(clienteId = null) {
  ...
  const DIAS = [
    { bit: 1,  label: "Lun" }, { bit: 2,  label: "Mar" }, { bit: 4,  label: "MiÃ©" },
    { bit: 8,  label: "Jue" }, { bit: 16, label: "Vie" }, { bit: 32, label: "SÃ¡b" },
    { bit: 64, label: "Dom" },
  ];
  ...
  const ESTADOS_LEGAL = ["Al corriente","En gestiÃ³n","Incumplimiento","Demanda","Promesa de pago","Cancelado"];
  // Segmentos: los que ya existen en Firestore + opciÃ³n para agregar nuevo
  ...
  body.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
      <div style="font-size:16px;font-weight:800;color:var(--text-primary)">
        ${clienteId ? "âœï¸ Editar cliente" : "âž• Nuevo cliente"}
      </div>
    ...
    <div style="display:grid;gap:16px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ${_field("Tipo de cultivo", `
          ...
          <datalist id="clf-cultivos-list">
            ...
            <option value="MaÃ­z"></option>
            <option value="Chile"></option>
            <option value="Pepino"></option>
            <option value="SandÃ­a"></option>
            <option value="MelÃ³n"></option>
            ...
            <option value="BrÃ³coli"></option>
            <option value="Lechuga"></option>
            <option value="EspÃ¡rrago"></option>
      ...
      <div>
        <div style="font-size:11px;font-weight:700;color:var(--text-sec);margin-bottom:6px">DÃ­as de visita</div>
        ...
        <select id="clf-frecuencia" style="${_inputStyle()}">
          ...
          ${_frecuencias.map((f, i) =>
            `<option value="${i}" data-label="${esc(f.label)}" data-dias="${f.dias}"
              ...
              ${esc(f.label)} (cada ${f.dias} dÃ­as)
            </option>`).join("")}
        </select>`) : ""}

      <!-- Cliente compartido -->
      ${_seccion("ðŸ”— Cliente compartido entre ingenieros")}
      <div style="border:1px solid var(--border);border-radius:8px;padding:12px;background:var(--surface-2)">
        <label style="display:flex;align-items:center;gap:10px;cursor:pointer;margin-bottom:10px">
          ...
          <span style="font-size:12px;font-weight:600;color:var(--text-primary)">
            Este cliente tiene parcelas o actividad en zonas de mÃ¡s de un ingeniero
          </span>
        </label>
  ...
  // BÃºsqueda de direcciÃ³n con Places Autocomplete
  _initGeoBuscador();
}
...
// _guardarFormCliente se expone desde _bindUI(); esta funciÃ³n
// es la implementaciÃ³n real invocada por window.ClientesUI._guardarFormCliente
async function _guardarFormCliente(clienteId) {
    ...
    // Validar duplicado por nombre exacto + zona (solo en alta, no en ediciÃ³n)
    if (!clienteId) {
      ...
      if (dup) {
        window.toast?.(`âš  Ya existe un cliente con ese nombre${zona ? " y zona" : ""}: ${dup.clienteId || dup.nombre}`, "warn");
        return;
      }
    }
    ...
    try {
      if (clienteId) {
        ...
        // Generar CLI-XXXXX: siguiente nÃºmero al mÃ¡ximo existente
      ...
      if (btn) { btn.disabled = false; btn.textContent = clienteId ? "ðŸ’¾ Guardar cambios" : "âœ… Crear cliente"; }
    }
  }

// â”€â”€ Ubicaciones de cliente â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const _TIPO_ICON = { parcela:"ðŸŒ¾", domicilio:"ðŸ ", invernadero:"ðŸ—", otro:"ðŸ“Œ" };
...
async function _cargarUbicaciones(clienteDocId) {
  ...
  try {
    ...
    list.innerHTML = snap.docs.map(d => {
      ...
      const icon  = _TIPO_ICON[u.tipo]  || "ðŸ“Œ";
      ...
      return `<div style="border:1px solid var(--border);border-radius:8px;padding:10px 12px;
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:10px;color:#9CA3AF;margin-top:2px">
            ${u.modificadoPor ? `Editado por ${esc(u.modificadoPor)}` : `Por ${esc(u.creadoPor||"â€”")}`}
          </div>
        </div>
        <div style="display:flex;flex-direction:column;gap:4px;flex-shrink:0">
          <button onclick="ClientesUI.navegarUbicacion(${u.lat},${u.lng})"
            ...
            style="padding:4px 10px;border:1px solid #1565C0;border-radius:5px;background:transparent;
              color:#1565C0;font-size:11px;cursor:pointer;font-weight:600">ðŸ—º Navegar</button>
          <button onclick="ClientesUI.abrirUbicacion('${esc(clienteDocId)}','${esc(uid)}')"
            style="padding:4px 10px;border:1px solid var(--border);border-radius:5px;background:transparent;
              color:var(--text-primary);font-size:11px;cursor:pointer">âœ Editar</button>
          <button onclick="ClientesUI.eliminarUbicacion('${esc(clienteDocId)}','${esc(uid)}')"
            style="padding:4px 10px;border:1px solid #FCA5A5;border-radius:5px;background:transparent;
              color:#DC2626;font-size:11px;cursor:pointer">ðŸ—‘</button>
        </div>
      </div>`;
    }).join("");
  } catch(e) {
...
// Estado del modal de ubicaciÃ³n
...
async function _abrirModalUbicacion(clienteDocId, ubicId) {
  ...
  // Cargar datos existentes si es ediciÃ³n
  let u = {};
  if (ubicId) {
    try {
  ...
  document.getElementById("cli-ubic-titulo").textContent = ubicId ? "Editar ubicaciÃ³n" : "Nueva ubicaciÃ³n";
  ...
  selZona.innerHTML = `<option value="">â€” Sin asignar â€”</option>` +
  ...
  try {
    ...
    const marker = new google.maps.Marker({
      ...
      title: "Arrastra para ajustar la posiciÃ³n",
      animation: google.maps.Animation.DROP,
    });

    if (!_ubicEdit.lat) marker.setVisible(false);

    // Click en mapa â†’ mover marker
...
async function _guardarUbicacion() {
  ...
  if (!lat || !lng) {
    errEl.textContent = "Selecciona la ubicaciÃ³n en el mapa.";
    errEl.style.display = "block"; return;
  }
  if (!tipo) {
    errEl.textContent = "Selecciona el tipo de ubicaciÃ³n.";
    errEl.style.display = "block"; return;
  }
  ...
  try {
    ...
    if (ubicId) {
      // EdiciÃ³n â€” guardar historial de cambios
      const oldData = {};
      try {
      ...
      if (cambios.length) {
        ad(col(db, "clientes", clienteDocId, "ubicaciones", ubicId, "historial"), {
          quien: alias, rol, cuÃ¡ndo: st(), cambios
        }).catch(() => {});
      }
    } else {
    ...
    window.toast?.(ubicId ? "UbicaciÃ³n actualizada." : "UbicaciÃ³n agregada.", "success");
    ...
    if (btn) { btn.disabled = false; btn.textContent = "Guardar ubicaciÃ³n"; }
  }
}

// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// ALTO/74 â€” Historial de pedidos del cliente en la ficha
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
async function _cargarPedidosRecientes(clienteDocId, cliente) {
  ...
  try {
    ...
    const fmtTs = ts => ts
      ...
      : "â€”";
    const ESTADO = { CONFIRMADO:"ðŸŸ¡ Conf.",ENTREGADO:"ðŸŸ¢ Entregado",CANCELADO:"ðŸ”´ Cancel.",
      EN_PROCESO:"ðŸ”µ En proceso",PENDIENTE:"âšª Pendiente" };
      ...
      <table class="data-table" style="font-size:11px">
        ...
        <tbody>
          ${rows.map(r => `<tr>
            ...
            <td style="font-family:monospace;font-size:10px">${esc(r.folio||r.id?.slice(-6)||"â€“")}</td>
            <td style="text-align:right;font-weight:700">${fmt.format(r.total||r.importe||0)}</td>
            <td>${ESTADO[r.estado]||esc(r.estado||"â€“")}</td>
          </tr>`).join("")}
        </tbody>
      </table>
    </div>`;
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// BAJO/44 â€” Log de visitas estructurado
// SubcolecciÃ³n: clientes/{id}/visitas_log
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  ...
  try {
    ...
    const fmtTs = ts => ts
      ? new Date(typeof ts==="number"?ts:ts.toMillis?.()??ts)
          .toLocaleDateString("es-MX",{day:"2-digit",month:"short",year:"numeric"})
      : "â€”";
    const TIPO_ICON = { Visita:"ðŸ¤", Llamada:"ðŸ“ž", WhatsApp:"ðŸ’¬", Email:"ðŸ“§", Virtual:"ðŸ’»", Otro:"ðŸ“‹" };
      ...
      ${rows.map(r => `
        <div style="border:1px solid var(--border);border-radius:8px;padding:10px 12px;background:var(--surface-2)">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
            <span style="font-size:16px">${TIPO_ICON[r.tipo]||"ðŸ“‹"}</span>
          ...
          ${r.proximaCita ? `<div style="font-size:10px;color:#7C3AED;margin-top:3px">
            ðŸ“… PrÃ³xima cita: ${esc(r.proximaCita)}</div>` : ""}
          <div style="font-size:10px;color:#94A3B8;margin-top:3px">Por ${esc(r.quienRegistro||"â€“")}</div>
        </div>`).join("")}
    </div>`;
  } catch(e) {
...
function _abrirFormVisita(clienteDocId) {
  ...
  const overlay = document.createElement("div");
  overlay.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:2100;display:flex;align-items:center;justify-content:center;padding:16px";
  overlay.innerHTML = `
    <div style="background:var(--surface);border-radius:14px;width:460px;max-width:100%;
      ...
      <div style="padding:16px 20px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:10px">
        ...
        <button id="_vis-close" style="background:none;border:none;cursor:pointer;font-size:18px;color:#64748B">âœ•</button>
      </div>
  ...
  overlay.querySelector("#_vis-guardar").addEventListener("click", async () => {
    ...
    try {
      ...
      // TambiÃ©n actualizar ultimaVisita en el cliente padre
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// MEDIO/58 â€” ClasificaciÃ³n ABC automÃ¡tica por volumen de compra
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
async function _calcularABC() {
  ...
  try {
    ...
    const snap = await getDocs(q2);
    const totales = {};  // clienteId â†’ total
    snap.docs.forEach(d => {
      const data = d.data();
    ...
    const limB = Math.ceil(n * 0.50);  // siguiente 30% â†’ hasta 50%
...
// â”€â”€ Fotos del cliente (tomadas en APK, guardadas en Firebase Storage) â”€â”€
async function _verFotosCliente(clienteId, nombre) {
  ...
  if (title) title.textContent = `ðŸ“· Fotos â€” ${nombre || clienteId}`;
  if (sub)   sub.textContent   = "Cargando fotos registradas desde el APKâ€¦";
  body.innerHTML = `<div style="text-align:center;padding:24px;color:var(--text-sec)">Cargandoâ€¦</div>`;
  modal.style.display = "flex";
  document.body.style.overflow = "hidden";
  ...
  try {
    ...
    for (const path of paths) {
      try {
        ...
        // TambiÃ©n busca en subcarpetas (ej. fechas)
    ...
    if (!items.length) {
      body.innerHTML = `
        <div style="text-align:center;padding:32px;color:var(--text-sec)">
          ...
          <div style="font-size:12px">Las fotos tomadas por el ingeniero en el APK aparecerÃ¡n aquÃ­ automÃ¡ticamente.</div>
